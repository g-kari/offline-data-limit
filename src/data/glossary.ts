...
export const GLOSSARY: GlossaryEntry[] = [
  // ── ストレージ API ──────────────────────────────────────────────────────────
  {
    ...
    description:
      "オリジン単位で管理されるキー・バリュー型のストレージ。setItem() / getItem() は同期実行でメインスレッドをブロックする。保存できるのは文字列のみで、バイナリを扱う場合は Base64 エンコードが必要。タブをまたいで共有され、ブラウザを閉じてもデータは消えない。上限は慣習的に約5MBとされているが仕様上の規定はなく[要出典]、UTF-16 エンコードの都合でバイナリを Base64 変換すると実質的な格納容量は約 3.75 MB 相当になる場合がある[要出典]。",
    references: [
      {
        label: "MDN: Window.localStorage",
        url: "https://developer.mozilla.org/ja/docs/Web/API/Window/localStorage",
      },
      {
        label: "Web Storage 仕様 (WHATWG)",
  ...
  },
  {
    ...
    description:
      "localStorage と同じAPIを持つが、スコープがタブ（ウィンドウ）単位。タブを閉じるとデータは即座に削除される。他のタブとは共有されないが、ページリロードでは保持される。window.open() で開いた子ウィンドウは親と同じ sessionStorage を共有する[要出典]。フォームの入力途中状態やマルチステップウィザードの一時保存に向いている。",
    references: [
      {
        label: "MDN: Window.sessionStorage",
        url: "https://developer.mozilla.org/ja/docs/Web/API/Window/sessionStorage",
      },
    ],
  },
  {
    ...
    description:
      "ブラウザ組み込みの非同期オブジェクトデータベース。構造化オブジェクト・Blob・ArrayBuffer を直接保存できる。インデックス検索・トランザクション・カーソルをサポート。API はコールバックベースだが idb ライブラリ等でPromise化できる。共有クォータプールからストレージを使用し、Chrome ではデバイスの空き容量の最大 80% まで利用できる。PWAのオフラインデータ保存の主力となるAPI。",
    references: [
      {
      ...
      {
        label: "Chrome ストレージクォータの概要",
        url: "https://developer.chrome.com/docs/web-platform/storage-overview",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "MDN: Cache",
        url: "https://developer.mozilla.org/ja/docs/Web/API/Cache",
      },
      {
        label: "web.dev: キャッシュ戦略",
  ...
  },
  {
    ...
    references: [
      ...
      {
        label: "web.dev: The origin private file system",
        url: "https://web.dev/articles/origin-private-file-system",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "SQLite Wasm 公式ドキュメント",
        url: "https://sqlite.org/wasm/doc/trunk/index.md",
      },
      {
        label: "wa-sqlite (GitHub)",
  ...
  },
  {
    ...
    references: [
      {
        label: "PGLite 公式ドキュメント",
        url: "https://pglite.dev/",
      },
      {
        label: "ElectricSQL (GitHub)",
  ...
  // ── 基本概念 ───────────────────────────────────────────────────────────────
  {
    ...
    references: [
      {
        label: "MDN: StorageManager.estimate()",
        url: "https://developer.mozilla.org/ja/docs/Web/API/StorageManager/estimate",
      },
      {
        label: "Chrome ストレージクォータの概要",
        url: "https://developer.chrome.com/docs/web-platform/storage-overview",
  ...
  },
  {
    ...
    description:
      "IndexedDB・Cache API・OPFS・SQLite/Wasm・PGLite の各APIは、ブラウザが管理する同一の「共有クォータプール」からストレージを消費する。あるAPIが大量にデータを書き込むと、他のAPIが使用できる残容量が減る。このためこれらのAPIは並列でベンチマークを実行すると正確な上限が測定できず、このツールでは逐次実行している。localStorage / sessionStorage はこのプールに含まれず、固定の別枠（約5MB）を持つ[要出典]。",
    references: [
      {
        label: "Storage Standard (WHATWG)",
  ...
  },
  {
    ...
    references: [
      {
        label: "MDN: オリジン",
        url: "https://developer.mozilla.org/ja/docs/Glossary/Origin",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "MDN: StorageManager.persist()",
        url: "https://developer.mozilla.org/ja/docs/Web/API/StorageManager/persist",
      },
      {
        label: "web.dev: Persistent storage",
  ...
  },
  {
    ...
    references: [
      {
        label: "Storage Standard: Quota and usage (WHATWG)",
        url: "https://storage.spec.whatwg.org/#quota-and-usage",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "web.dev: Progressive Web Apps",
        url: "https://web.dev/articles/progressive-web-apps",
      },
      {
        label: "MDN: Progressive Web Apps",
        url: "https://developer.mozilla.org/ja/docs/Web/Progressive_web_apps",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    description:
      "単位時間あたりに書き込めるデータ量（MB/秒）。同期APIの localStorage は比較的高いスループットを示すが、大量書き込みでメインスレッドをブロックする。非同期のIndexedDB/Cache APIはスループットは落ちるが UIをブロックしない。OPFS の同期アクセスハンドルはネイティブファイルI/Oに近い高スループットが出る傾向がある[要出典]。",
  },
  {
    id: "chunk",
    term: "チャンク",
  ...
  },
  {
    ...
    description:
      "ストレージAPIへの書き込みがブラウザのクォータを超えた際にスローされる DOMException。name プロパティが 'QuotaExceededError'（FirefoxはNS_ERROR_DOM_QUOTA_REACHED）になる[要出典]。このツールはこの例外をキャッチしてチャンクサイズを半減させるバイナリサーチを行う。",
    references: [
      {
        label: "MDN: DOMException",
        url: "https://developer.mozilla.org/ja/docs/Web/API/DOMException",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    description:
      "navigator.storage.estimate() を呼び出すと { quota, usage } オブジェクトが返る。quota は「利用可能な推定ストレージ容量」で、ブラウザがデバイスのディスク空き容量などを元に計算した推定値。実際に書き込める量（実測上限）とは差が生じる場合がある。iOS Safari では実態より大きな値が返ることがある[要出典]。",
    references: [
      {
        label: "MDN: StorageManager.estimate()",
        url: "https://developer.mozilla.org/ja/docs/Web/API/StorageManager/estimate",
  ...
  },
  {
    ...
    description:
      "ベンチマーク完了後、実際に書き込んだデータを読み返して正しく保存されているか確認する処理。検証OKなら書き込みと読み取りが整合している。検証NGの場合、APIがエラーなく書き込みを受け付けたように見えて実際にはデータが保存できていない可能性がある（一部のモバイルブラウザで報告されている挙動）[要出典]。",
  },
    ...
    references: [
      {
        ...
        url: "https://developer.mozilla.org/ja/docs/WebAssembly",
      },
      {
        label: "WebAssembly 公式",
  ...
  },
  {
    ...
    references: [
      ...
      {
        label: "web.dev: Service workers の概要",
        url: "https://web.dev/articles/service-workers-basics",
      },
    ],
  },
  {
    ...
    description:
      "メインスレッドと Web Worker の間で同じメモリ領域を共有するためのオブジェクト。Spectre 脆弱性への対策として、使用するには COOP（Cross-Origin-Opener-Policy: same-origin）と COEP（Cross-Origin-Embedder-Policy: require-corp）の両方のレスポンスヘッダーが必要。wa-sqlite の OPFS VFS が内部で使用している。",
    references: [
      {
        label: "MDN: SharedArrayBuffer",
        url: "https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer",
      },
      {
        label: "web.dev: SharedArrayBuffer の更新",
  ...
  },
  {
    ...
    references: [
      {
        label: "web.dev: COOP と COEP を使用してウェブサイトを「クロスオリジン分離」にする",
        url: "https://web.dev/articles/coop-coep",
      },
      {
        label: "MDN: Cross-Origin-Opener-Policy",
        url: "https://developer.mozilla.org/ja/docs/Web/HTTP/Headers/Cross-Origin-Opener-Policy",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "SQLite VFS ドキュメント",
        url: "https://www.sqlite.org/vfs.html",
      },
    ],
  },
  {
    ...
    references: [
      {
        label: "MDN: FileSystemSyncAccessHandle",
        url: "https://developer.mozilla.org/en-US/docs/Web/API/FileSystemSyncAccessHandle",
      },
    ],
  },
  ...
  // ── ブラウザ・プラットフォーム ──────────────────────────────────────────────
  {
    ...
    description:
      "Intelligent Tracking Prevention（インテリジェント・トラッキング・プリベンション）の略。Apple Safari に搭載されたプライバシー保護機能。7日間ユーザーがサイトにアクセスしないと、IndexedDB・Cache API・localStorage などのストレージデータが自動削除される。navigator.storage.persist() で永続化を要求しても、ユーザーが明示的に許可しない限りiOSでは通常削除対象になる[要出典]。",
    references: [
      {
        label: "WebKit Blog: Full Third-Party Cookie Blocking and More",
        url: "https://webkit.org/blog/10218/full-third-party-cookie-blocking-and-more/",
      },
      {
        label: "WebKit Blog: Storage access policy",
  ...
  },
  {
    ...
    references: [
      {
        label: "Chrome DevTools: ストレージの表示と変更",
  ...
  },
  {
    ...
    references: [
      {
        label: "Chrome DevTools: Android デバイスでのリモートデバッグ",
  ...
  },
  {
    ...
    description:
      "Storage API の一部。{ quota: number, usage: number, usageDetails?: {...} } を解決する Promise を返す。quota は利用可能と推定されるバイト数、usage は現在使用中のバイト数。usageDetails では IndexedDB / Cache API / Service Worker などの内訳を確認できる（ブラウザによって対応状況が異なる）[要出典]。返り値はあくまで推定値であり実際に書き込める量とは異なる場合がある。",
    references: [
      {
        label: "MDN: StorageManager.estimate()",
        url: "https://developer.mozilla.org/ja/docs/Web/API/StorageManager/estimate",
  ...
  },
  {
    ...
    description:
      "Storage API の一部。ストレージの永続化をブラウザに要求し、許可されたかどうかを示す boolean を解決する Promise を返す。永続化が許可されると、ブラウザはストレージプレッシャー時にそのオリジンのデータを自動削除しなくなる。ChromeではPWAとしてインストールされているか、ユーザーがサイトをブックマークしているかなどの条件で自動的に許可されることもある[要出典]。iOSでは通常ユーザーの明示的な操作が必要[要出典]。",
    references: [
      {
        label: "MDN: StorageManager.persist()",
        url: "https://developer.mozilla.org/ja/docs/Web/API/StorageManager/persist",
